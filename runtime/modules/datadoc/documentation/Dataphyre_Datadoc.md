...
2. **Universal Static Documentation Portal**
   ...
   - **`dev/tools/datadoc_docs.php`**: Publishes either a manually mounted corpus or the automatically discovered Dataphyre workspace through Datadoc. It is preview-first and writes only when `--write` is explicit.
