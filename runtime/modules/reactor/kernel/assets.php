...
$client_assets=dirname(__DIR__).'/Framework/Client/ReactorClientAssets.php';
if(is_file($client_assets)){
	require_once($client_assets);
}

$asset=(string)(\dataphyre\routing::$bindings['asset'] ?? $_GET['asset'] ?? basename((string)parse_url((string)($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH)));
$content=class_exists('\Dataphyre\Reactor\ReactorClientAssets', false) ? \Dataphyre\Reactor\ReactorClientAssets::assetContent($asset) : null;
if(!is_array($content)){
	http_response_code(404);
	header('Content-Type: text/plain; charset=utf-8');
	header('Cache-Control: no-store');
