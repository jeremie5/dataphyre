...
final class McpToolArguments {
	...
	private static function engagedStringValue(string $tool,string $name): string {
		return match($name){
			...
			'path'=>match(true){
				str_contains($tool,'config_shape'),str_contains($tool,'config_value')=>'dataphyre/config/storage.php',
			...
			'config_path'=>str_contains($tool,'sql_') ? 'dataphyre/config/sql.php' : 'dataphyre/config/storage.php',
