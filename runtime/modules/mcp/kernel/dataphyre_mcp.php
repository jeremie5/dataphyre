...
final class dataphyre_mcp_server {
	...
	private function prompt_text(string $name): string {
		...
		return match($name){
			...
			'dataphyre_release_triage'=>"Run dataphyre_release_check with the application project root, Dataphyre application id, and environment. Treat likely_to_deploy as the deterministic local prediction: fix each configuration, dependency, or verification failure, including an application-resolved managed database identity mismatch, before release. Dataphyre Cloud must run the same fixed preflight command inside the exact built candidate and preserve source, image, environment, and traffic identity before promotion. Never substitute an application release script or caller-selected executable.",
