	...
	private function mcp_tool_boundary_map(array $tool_names): array {
		$catalog=[
			...
			'dataphyre_release_check'=>[
				...
				'claim_boundary'=>'Deterministic local prediction from fixed configuration, migration dry-run, application-resolved managed database identity, application boot, GET /health, realtime callback registration, and scheduler definition registration checks; a hosting control plane must repeat it inside the exact built candidate and prove the three fixed pools, scheduler callback execution, and authenticated event roundtrip before promotion.',
				'not_app_behavior_proof'=>true,
			],
			'dataphyre_release_triage_summary'=>[
				'audience_scope'=>'publication_validation_not_ordinary_app_work',
