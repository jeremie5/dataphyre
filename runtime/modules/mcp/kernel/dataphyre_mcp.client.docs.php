...
trait dataphyre_mcp_client_docs_surfaces {
	...
	private function mcp_docs_coverage_report(): array {
		$docs_path='dataphyre/runtime/modules/mcp/documentation/Dataphyre_MCP.md';
		...
		$gaps=$this->mcp_docs_coverage_gaps($docs,$guidelines,$tools,$resources,$prompts,$skills,$safety_terms);
		return [
			...
			'counts'=>[
				...
				'skills'=>count($skills),
				'missing_tools'=>count($gaps['tools']),
				'missing_core_resources'=>count($gaps['core_resources']),
				'missing_prompts'=>count($gaps['prompts']),
				'missing_skills'=>count($gaps['skills']),
				'missing_safety_terms'=>count($gaps['safety_terms']),
			],
			'complete'=>array_filter($gaps)===[],
			'missing'=>$gaps,
	...
	/**
	 * Compares documentation text with independently supplied public catalogs.
	 *
	 * @param list<string> $tools
	 * @param list<string> $resources
	 * @param list<string> $prompts
	 * @param list<string> $skills
	 * @param list<string> $safety_terms
	 * @return array{tools:list<string>,core_resources:list<string>,prompts:list<string>,skills:list<string>,safety_terms:list<string>}
	 */
	private function mcp_docs_coverage_gaps(string $docs,string $guidelines,array $tools,array $resources,array $prompts,array $skills,array $safety_terms): array {
		$missing_tools=array_values(array_filter($tools,static fn(string $tool): bool=>!str_contains($docs,'`'.$tool.'`') && !str_contains($docs,$tool)));
		$missing_resources=array_values(array_filter($resources,static fn(string $resource): bool=>!str_starts_with($resource,'dataphyre://doc/') && !str_contains($docs,$resource)));
		$missing_prompts=array_values(array_filter($prompts,static fn(string $prompt): bool=>!str_contains($docs,$prompt) && !str_contains($guidelines,$prompt)));
		$missing_skills=array_values(array_filter($skills,static fn(string $skill): bool=>!str_contains($docs,$skill) && !str_contains($guidelines,$skill)));
		$combined=strtolower($docs."\n".$guidelines);
		$missing_safety=array_values(array_filter($safety_terms,static fn(string $term): bool=>!str_contains($combined,strtolower($term))));
		return [
			'tools'=>$missing_tools,
