...
Dpanel's browser UI is embedded in Dataphyre Flightdeck at `/dataphyre/dpanel`.

The Flightdeck surface provides scoped scan actions for runtime modules, app modules, or both. Results render as Flightdeck cards with summary metrics and expandable diagnostic details. Flightdeck owns authentication, layout, navigation, and form CSRF; Dpanel continues to own module validation, trace capture, and unit-test execution. The Flightdeck renderer preserves Dpanel's richer diagnostics: `info` entries are shown as first-class messages, captured tracelog batches render as expandable tables with severity summaries, and raw diagnostic payloads stay available when an entry has no conventional message field.

Dpanel explicitly runs module diagnostic files during a Flightdeck scan, even when the module's main file was already loaded by the current request. This keeps the embedded view aligned with the standalone diagnostic behavior where module-specific `*.diagnostic.php` checks contributed additional findings.
