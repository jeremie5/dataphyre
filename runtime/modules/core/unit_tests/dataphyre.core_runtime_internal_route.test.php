...
test('self-hosted scheduler callbacks are intercepted before application routes and require the current signed v2 headers',static function(Context $t): void {
	...
	$server=[
		...
		'HTTP_X_DATAPHYRE_SCHEDULER_KEY'=>$signature,
		'HTTP_X_DATAPHYRE_SCHEDULER_BUDGET_MS'=>'30000',
		'HTTP_X_DATAPHYRE_SCHEDULER_ISSUED_AT'=>(string)$issuedAt,
	];
	...
	$handled=$runtime->invoke('boot_internal_runtime_route',$definition,[
		'server'=>$server,
		'verify'=>static function(string $token,string $name,string $candidateClaim,int $budget,int $candidateIssuedAt)use(&$verified,$signature): bool {
			$verified[]=[$token,$name,$candidateClaim,$budget,$candidateIssuedAt];
			return $token===$signature && $name==='tenant-beta.lifecycle'
				&& $candidateClaim===str_repeat('c',64) && $budget===30000 && $candidateIssuedAt>0;
		},
	...
	$t->isTrue($handled);
	$t->same([[$signature,'tenant-beta.lifecycle',$claim,30000,$issuedAt]],$verified);
	...
	$invalidBudget=$server;
	$invalidBudget['HTTP_X_DATAPHYRE_SCHEDULER_BUDGET_MS']='300001';
	$t->isTrue($runtime->invoke('boot_internal_runtime_route',$definition,[
		'server'=>$invalidBudget,
		'verify'=>static fn(): bool=>true,
		'respond'=>static function(int $status,string $body)use(&$responses): void {$responses[]=[$status,$body];},
	]));
	$post=$server;
	...
	$t->same([[404,'Not found'],[404,'Not found'],[404,'Not found']],$responses);
	$t->isFalse($runtime->invoke('boot_internal_runtime_route',$definition,['server'=>['REQUEST_URI'=>'/ordinary-route']]));
})->tag('core','runtime','scheduler','self-hosted','signature','fail-closed');
