...
final class ApplicationReleasePreflightEvidence
{
	...
	private static function release_preflight_claim_boundary(): string {
		return 'This verdict covers local configuration bootstrap, the native PostgreSQL migration dry-run or isolated SQL-only SQLite apply when declared, application startup against the same database state, GET /health, and deterministic realtime callback, scheduler definition, and registered table-definition inventories. A release platform must run this same command inside the exact candidate image and separately prove the ordered schema stages (declared application migrations first, then fixed registered-table materialization), the four direct runtime child identities and eight FPM workers, fixed warm-latency and concurrent-request budgets, scheduler callback execution and cadence, a framework listener roundtrip, execution and strict invalid-Origin rejection by every registered application authorization callback, WebSocket ping/pong and close, signal lifecycle, persistent application-data binding, and source, image, environment, database, and traffic identity.';
	}
