...
/** Resolves standalone CLI guards before any Panel runtime dependency is loaded. */
function panel_field_catalog_early_result(array $argv, string $sapi): ?int {
	if($sapi!=='cli'){
...
if(!defined('DATAPHYRE_PANEL_FIELD_CATALOG_EMBEDDED')){ $earlyResult=panel_field_catalog_early_result($argv ?? [], PHP_SAPI); if($earlyResult!==null){ exit($earlyResult); } }

$modules_root=dirname(__DIR__, 2);
require_once $modules_root.'/core/kernel/autoloader.php';
\dataphyre\autoloader::register($modules_root);
\dataphyre\autoloader::register_prefixes(['Dataphyre\\Panel\\'=>dirname(__DIR__).'/Framework']);
...
/** @return list<string> Complete route-free field catalog failures. */
function panel_field_catalog_run(): array {
$failures=[];
...
panel_field_catalog_assert(str_contains($display_html, '<div class="dp-panel-field') && str_contains($display_html, 'dp-panel-grid-item-auto') && str_contains($display_html, 'dp-panel-field-display') && str_contains($display_html, 'data-dp-panel-display-field="1"') && str_contains($display_html, '<strong>Reserved</strong>') && !str_contains($display_html, '<label class="dp-panel-field'), 'Renderer emits display fields as non-label safe HTML blocks.', $failures);
...
for($relationship_index=1; $relationship_index<=13; $relationship_index++){
	$relationship_options[(string)$relationship_index]='User '.$relationship_index;
}
$relationship_html=panel_field_catalog_render_control('owner_id', Field::make('owner_id')->relationship('users', $relationship_options, 'name', 'id'), '2');
...
panel_field_catalog_assert(str_contains($radio_html, 'role="radiogroup"') && str_contains($radio_html, 'data-dp-panel-choice-list="single"') && str_contains($radio_html, 'data-dp-panel-choice-columns="2"') && str_contains($radio_html, 'checked'), 'Renderer emits accessible radio choice card metadata.', $failures);
...
panel_field_catalog_assert(str_contains($toggle_html, 'dp-panel-switch-track') && str_contains($toggle_html, '<strong data-dp-panel-switch-label="1">Active</strong>') && str_contains($toggle_html, 'data-dp-panel-switch-off="Paused"'), 'Renderer emits toggle track and label metadata.', $failures);
...
$upload_endpoint=PanelConfig::uploadUrl();
panel_field_catalog_assert(str_contains($uploader_html, 'data-dp-panel-uploader="1"') && str_contains($uploader_html, 'data-dp-panel-uploader-endpoint="'.htmlspecialchars($upload_endpoint, ENT_QUOTES, 'UTF-8').'"') && str_contains($uploader_html, 'data-dp-panel-uploader-total'), 'Renderer emits custom uploader shell metadata.', $failures);
panel_field_catalog_assert(str_contains($uploader_html, 'data-dp-panel-uploader-delete-endpoint="'.htmlspecialchars($upload_endpoint, ENT_QUOTES, 'UTF-8').'"'), 'Renderer emits uploader delete endpoint metadata.', $failures);
...
foreach(['dpPanelEditorRefreshEmptyState', 'dpPanelEditorVisualHasContent', 'data-dp-panel-editor-placeholder'] as $needle){
	$rich_editor_html=panel_field_catalog_render_control('body', Field::make('body')->richEditor()->placeholder('Write body'));
	panel_field_catalog_assert(str_contains($script.$component_css.$rich_editor_html, $needle), 'Panel assets contain rich editor empty placeholder support for '.$needle.'.', $failures);
}
...
/** Executes the field catalog CLI without forcing embedded test callers to exit. */
function panel_field_catalog_main(array $argv, ?array $failures=null, ?string $sapi=null): int {
	$sapi ??=PHP_SAPI;
	$earlyResult=panel_field_catalog_early_result($argv, $sapi);
...
if(!defined('DATAPHYRE_PANEL_FIELD_CATALOG_EMBEDDED')){ exit(panel_field_catalog_main($argv ?? [])); }
