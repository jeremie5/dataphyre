...
test('signed logs survive restart and receipts bind sequence inclusion consistency and witness quorum',static function(Context $t):void{
	...
	$log=new PanelPackageTransparencyLog($root,'shopiro_public_log',$authority['primary'],$authority['signer'],$clock);
	$first=$log->append('registry_index',['registry'=>'shopiro_packages','sequence'=>1,'digest'=>str_repeat('a',64)]);
	...
	$restarted=new PanelPackageTransparencyLog($root,'shopiro_public_log',$authority['primary'],$authority['signer'],$clock);
...
test('stateful verifier rejects rollback split views stale heads missing consistency and untrusted witnesses',static function(Context $t):void{
	$authority=dp_panel_transparency_authority();$now='2026-07-16T12:00:00.000000Z';$clock=static function()use(&$now):string{return$now;};
	$root=$t->tempDirectory('panel-package-transparency-verifier');$log=new PanelPackageTransparencyLog($root.'/honest','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);
	$subject1=['registry'=>'shopiro_packages','sequence'=>1,'digest'=>str_repeat('1',64)];$receipt1=$log->append('registry_index',$subject1)->witness($authority['witness'],$authority['signer']);
	$verifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'allowed_witnesses'=>[$authority['witness']],'required_witnesses'=>1,'clock'=>$clock]);
	...
	$rogue=new PanelPackageTransparencyLog($root.'/rogue','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);
	...
	$restored=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],$trusted1['checkpoints'],['allow_trust_on_first_use'=>false,'allowed_witnesses'=>[$authority['witness']],'required_witnesses'=>1,'clock'=>$clock]);
	...
	$consistencyRequired=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],$trusted1['checkpoints'],['clock'=>$clock]);$t->isFalse($consistencyRequired('package_release',$subject2,$log->receipt(1)->jsonSerialize()));
	$t->isFalse($restored('package_release',$subject2,[]));
	$t->throws(static fn()=>$restored->restore(['checkpoints'=>['shopiro_public_log'=>['tree_size'=>0,'root_hash'=>'invalid','issued_at'=>$now]]]),InvalidArgumentException::class);
	$invalidClock=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>static fn():array=>[]]);$t->throws(static fn()=>$t->nonPublic($invalidClock)->invoke('now'),UnexpectedValueException::class);
	$now='2026-07-20T12:00:00.000000Z';$stale=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>$clock,'max_checkpoint_age_seconds'=>60]);
...
test('transparency events reject secrets malformed schemas and event sequence substitution',static function(Context $t):void{
	$authority=dp_panel_transparency_authority();$root=$t->tempDirectory('panel-package-transparency-rejections');$log=new PanelPackageTransparencyLog($root,'shopiro_public_log',$authority['primary'],$authority['signer'],static fn():string=>'2026-07-16T12:00:00.000000Z');
...
test('durable trust network propagates contiguous revocations and evidence-based publisher profiles across restart',static function(Context $t):void{
	...
	$log=new PanelPackageTransparencyLog($root.'/log','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);
	$verifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>$clock]);
	$network=new PanelPackageMarketplaceTrustNetwork($root.'/network',$verifier,$clock,3600);
	$attestation=['attestation_id'=>'attestation_identity_1','publisher'=>'shopiro','issuer'=>'independent_lab','category'=>'identity','signal'=>'verified','evidence_hash'=>str_repeat('a',64),'issued_at'=>$now,'valid_until'=>'2026-08-16T12:00:00.000000Z'];
	$network->ingest($log->append('publisher_attestation',$attestation));
	$t->same($verifier,$network->verifier());$publishers=$network->publishers();$t->same($network,$publishers->network());$profile=$publishers->profile('shopiro');$t->same('observed',$profile->status());$t->isTrue($profile->eligible());$t->isTrue($profile->complete());$t->same(null,$profile->jsonSerialize()['score']);$t->same('shopiro',$publishers->assertEligible('shopiro')->publisher());$t->same('panel_package_publisher_trust_registry_manifest',$publishers->jsonSerialize()['type']);

	$now='2026-07-16T12:01:00.000000Z';$revocation=['revocation_id'=>'revocation_orders_1','scope'=>'version','publisher'=>'shopiro','package'=>'orders','version'=>'1.0.0','reason'=>'supply_chain_incident','effective_at'=>$now];
	$network->ingest($log->append('revocation',$revocation,['consistency_from_size'=>1]));
	$revocations=$network->revocations();$t->same($network,$revocations->network());$decision=$revocations->decision('package',['publisher'=>'shopiro','package'=>'orders','version'=>'1.0.0']);$t->isTrue($decision->revoked());$t->isFalse($decision->allowed());$t->throws(static fn()=>$decision->assertAllowed(),LogicException::class);$t->isTrue($revocations('package',['publisher'=>'shopiro','package'=>'orders','version'=>'1.0.0'])['revoked']);$t->same('panel_package_revocation_registry_manifest',$revocations->jsonSerialize()['type']);
	$t->isTrue($network->revocations()->decision('package',['publisher'=>'shopiro','package'=>'orders','version'=>'2.0.0'])->allowed());
	...
	$freshVerifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>false,'clock'=>$clock]);
	$restarted=new PanelPackageMarketplaceTrustNetwork($root.'/network',$freshVerifier,$clock,3600);
	$t->isTrue($restarted->revocations()->decision('version',['publisher'=>'shopiro','package'=>'orders','version'=>'1.0.0'])->revoked());
	$t->same('observed',$restarted->publishers()->profile('shopiro')->status());
...
test('publisher evidence can restrict block expire withdraw and supersede without inventing a reputation score',static function(Context $t):void{
	$authority=dp_panel_transparency_authority();$now='2026-07-16T12:00:00.000000Z';$clock=static function()use(&$now):string{return$now;};$root=$t->tempDirectory('panel-package-publisher-evidence');$log=new PanelPackageTransparencyLog($root.'/log','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);$verifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>$clock]);$network=new PanelPackageMarketplaceTrustNetwork($root.'/network',$verifier,$clock,3600);
	$make=static fn(string $id,string $category,string $signal,string $hash,array $extra=[]):array=>['attestation_id'=>$id,'publisher'=>'shopiro','issuer'=>'security_lab','category'=>$category,'signal'=>$signal,'evidence_hash'=>$hash,'issued_at'=>$now,'valid_until'=>'2026-07-17T12:00:00.000000Z']+$extra;
	$network->ingest($log->append('publisher_attestation',$make('identity_ok','identity','verified',str_repeat('1',64))));
	$now='2026-07-16T12:01:00.000000Z';$network->ingest($log->append('publisher_attestation',$make('maintenance_warning','maintenance','warning',str_repeat('2',64)),['consistency_from_size'=>1]));$t->same('restricted',$network->publishers()->profile('shopiro')->status());
	$now='2026-07-16T12:02:00.000000Z';$network->ingest($log->append('publisher_attestation',$make('security_failed','security','failed',str_repeat('3',64)),['consistency_from_size'=>2]));$t->same('blocked',$network->publishers()->profile('shopiro')->status());
	$now='2026-07-16T12:03:00.000000Z';$network->ingest($log->append('publisher_attestation',$make('security_withdrawn','security','withdrawn',str_repeat('4',64),['supersedes'=>'security_failed']),['consistency_from_size'=>3]));$t->same('restricted',$network->publishers()->profile('shopiro')->status());
	$now='2026-07-16T12:04:00.000000Z';$network->ingest($log->append('publisher_attestation',$make('maintenance_replaced','maintenance','verified',str_repeat('5',64),['supersedes'=>'maintenance_warning']),['consistency_from_size'=>4]));$profile=$network->publishers()->profile('shopiro');$t->same('observed',$profile->status());$t->same(2,$profile->jsonSerialize()['signals']['verified']);
	$now='2026-07-18T12:00:00.000000Z';$expired=$network->publishers()->profile('shopiro');$t->same('unknown',$expired->status());$t->isFalse($expired->eligible());
});

test('trust network fails closed on gaps stale checkpoints and malformed domain events',static function(Context $t):void{
	$authority=dp_panel_transparency_authority();$now='2026-07-16T12:00:00.000000Z';$clock=static function()use(&$now):string{return$now;};$root=$t->tempDirectory('panel-package-trust-fail-closed');$log=new PanelPackageTransparencyLog($root.'/log','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);$verifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>$clock,'max_checkpoint_age_seconds'=>3600]);$network=new PanelPackageMarketplaceTrustNetwork($root.'/network',$verifier,$clock,3600);
	$t->isFalse($network->revocations()->decision('publisher',['publisher'=>'shopiro'])->allowed());$t->same('unknown',$network->publishers()->profile('shopiro')->status());
	$first=$log->append('registry_index',['registry'=>'shopiro_packages','sequence'=>1,'digest'=>str_repeat('a',64)]);$second=$log->append('package_release',['package'=>'orders','version'=>'1.0.0','artifact_sha256'=>str_repeat('b',64)],['consistency_from_size'=>1]);
	...
	$t->throws(static fn()=>PanelPackageMarketplaceTrustNetwork::normalizeAttestationSubject(['attestation_id'=>'bad','publisher'=>'shopiro','issuer'=>'lab','category'=>'security','signal'=>'withdrawn','evidence_hash'=>str_repeat('a',64),'issued_at'=>$now,'valid_until'=>'2026-07-17T14:00:00.000000Z']),InvalidArgumentException::class);
});
...
test('marketplace governance rechecks publisher evidence and revocation after independent approval',static function(Context $t):void{
	$authority=dp_panel_transparency_authority();$now='2026-07-16T12:00:00.000000Z';$clock=static function()use(&$now):string{return$now;};$root=$t->tempDirectory('panel-marketplace-governance-recheck');$log=new PanelPackageTransparencyLog($root.'/log','shopiro_public_log',$authority['primary'],$authority['signer'],$clock);$verifier=new PanelPackageTransparencyVerifier($authority['verifier'],['shopiro_public_log'],[],['allow_trust_on_first_use'=>true,'clock'=>$clock]);$network=new PanelPackageMarketplaceTrustNetwork($root.'/network',$verifier,$clock,3600);
	$attestation=['attestation_id'=>'shopiro_identity','publisher'=>'shopiro','issuer'=>'independent_lab','category'=>'identity','signal'=>'verified','evidence_hash'=>str_repeat('a',64),'issued_at'=>$now,'valid_until'=>'2026-08-16T12:00:00.000000Z'];$network->ingest($log->append('publisher_attestation',$attestation));
	...
	$governance=new \Dataphyre\Panel\PanelMarketplaceGovernance($policy,2,['filesystem.*'],$clock,$network->revocations(),$network->publishers());$package=\Dataphyre\Panel\PanelPackageManifest::make('governed_pack')->version('1.0.0')->support('owner','shopiro');$trust=new \Dataphyre\Panel\PanelPackageTrustReport([],['trusted'=>1,'blocked'=>0]);$submission=['permissions'=>['orders.read'],'sbom'=>[['component'=>'safe','vulnerability_severity'=>'none']],'provenance'=>['builder'=>'trusted_builder','source_digest'=>str_repeat('b',64)],'compatibility'=>['passed'=>true]];
	...
	$now='2026-07-16T12:01:00.000000Z';$revocation=['revocation_id'=>'governed_pack_incident','scope'=>'version','publisher'=>'shopiro','package'=>'governed_pack','version'=>'1.0.0','reason'=>'supply_chain_incident','effective_at'=>$now];$network->ingest($log->append('revocation',$revocation,['consistency_from_size'=>1]));$t->throws(static fn()=>$governance->activation('governed_pack'),LogicException::class);
