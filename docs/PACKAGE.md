...
- `extra.dataphyre.release-manifest`: `RELEASE_MANIFEST.json` in a prepared public export root.
  Source checkouts keep the schema and human-readable contract under `docs/`;
  the JSON manifest is generated by the public export tooling.
- `extra.dataphyre.agent-boundary`: machine-readable guidance that the default
  ...
  maintainer work only, `agentic-enterprise-contract` points to
  ...
  `acceptance_review_plan`, and `local_convention_probe`), and dev tools,
  `dataphyre_mcp_verify_all`, hot-path benchmarks, and runtime-internal edits
