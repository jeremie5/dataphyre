...
| `php runtime/modules/sql/kernel/managed_seeds.php --project-root=/app --application=<id> --environment=<id> --profile=<name> --data-environment=<name> --allow-demo=<0\|1>` | Fixed host-managed PostgreSQL profile apply. A privileged one-shot broker supplies this exact argv after pruning every unselected typed managed database binding, captures all child output, and forwards only one validated bounded `dataphyre.managed_seed_apply.v1` object. Callers cannot select a mode, script, path, seed id, cluster, ledger, rollback, reset, or shell command. Convergence, environment unwind, and immediate-query checks precede commit; only missing evidence after commit acknowledgement is outcome-unknown and is resolved by idempotently retrying the same immutable profile. |
