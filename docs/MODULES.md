...
modules are optional and load only when the selected application's flight sheet
enables them and their files can then be resolved.
...
| `cache` | optional | No | [docs](../runtime/modules/cache/documentation/Dataphyre_Cache.md) | Fail-open Memcached cache facade with an explicit shared-backend health contract. |
...
- Optional modules must be listed in `bootstrap.modules.enabled`; directory
  presence never opts a module into the runtime.
- `bootstrap.modules.disabled` wins over enabled entries. Application-level
  source can override an enabled shared module's implementation, but cannot
  enable an omitted module merely by existing on disk.
